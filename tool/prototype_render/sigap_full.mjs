...
await page.goto('http://localhost:7375/'+'assets/works/SIGAP - Panic Button/SIGAP Mobile (standalone).html'.split('/').map(encodeURIComponent).join('/'),{waitUntil:'networkidle'}).catch(()=>{});
await sleep(3000);
...
await click('Menuju lokasi'); await snap('03-dispatch.png');
