...
await new Promise(r=>server.listen(7377,r));
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
const browser=await chromium.launch({channel:'chrome',headless:true});
const g=await browser.newPage({viewport:{width:1440,height:1000},deviceScaleFactor:1.5});
await g.goto('http://localhost:7377/',{waitUntil:'load'}).catch(()=>{});
await sleep(8000); await g.mouse.move(720,500); await g.mouse.click(720,500); await sleep(500);
for(let i=0;i<4;i++){await g.mouse.wheel(0,760);await sleep(700);} await sleep(1000);
await g.screenshot({path:path.join(OUT,'grid-row1.png')}); await g.close();
const p=await browser.newPage({viewport:{width:1440,height:1120},deviceScaleFactor:1});
await p.goto('http://localhost:7377/#/work/elssa',{waitUntil:'networkidle'}).catch(()=>{});
await sleep(8000); await p.screenshot({path:path.join(OUT,'elssa-cs-full.png')}); await p.close();
