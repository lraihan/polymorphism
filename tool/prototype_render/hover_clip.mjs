...
await new Promise(r=>server.listen(7365,r));
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
const browser=await chromium.launch({channel:'chrome',headless:true});
const page=await browser.newPage({viewport:{width:1440,height:1000},deviceScaleFactor:1});
await page.goto('http://localhost:7365/',{waitUntil:'load'}).catch(()=>{});
await sleep(8000);
...
await sleep(800);
await page.mouse.move(420,430,{steps:8}); await sleep(1500);
await page.screenshot({path:path.join(OUT,'hov-roast-clip.png'), clip:{x:120,y:84,width:690,height:400}});
console.log('done');
